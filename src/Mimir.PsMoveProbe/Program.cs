...
foreach (var device in devices)
{
    ...
    {
        var result = holdSeconds >= 0.0
            ? await WindowsHid.TryHoldLedAsync(
                device,
                rgb,
            ...
            : await WindowsHid.TryWriteLedPulseTrainAsync(
                device,
                rgb,
                Math.Max(1, pulseCount),
                TimeSpan.FromMilliseconds(Math.Max(0, pulseMs)),
                TimeSpan.FromMilliseconds(Math.Max(1, intervalMs)),
                eventLog).ConfigureAwait(false);
...
internal static class WindowsHid
{
    ...
    public static async Task<string> TryHoldLedAsync(
        HidDeviceInfo device,
        string rgb,
        TimeSpan duration,
        TimeSpan refresh,
        ...
        bool turnOffOnExit)
    {
        ...
        Console.CancelKeyPress += (_, eventArgs) =>
        {
            ...
            if (turnOffOnExit)
            {
                _ = TryWriteLed(device, 0, 0, 0);
            }

            Environment.Exit(0);
        };

        var eventId = $"starfire-usb-move-hold:{DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()}";
        var stopwatch = Stopwatch.StartNew();
        var writes = 0;
        while (duration == TimeSpan.Zero || stopwatch.Elapsed < duration)
        {
            var started = NowNs();
            var result = TryWriteLed(device, r, g, b);
            var completed = NowNs();
            AppendPulseEvent(eventLogPath, eventId, writes, "hold", r, g, b, started, completed, result);
        ...
        if (turnOffOnExit)
        {
            ...
            if (!result.StartsWith("ok", StringComparison.OrdinalIgnoreCase))
            {
                return $"off failed after {writes} writes: {result}";
            }
        }
