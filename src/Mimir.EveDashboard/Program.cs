...
internal sealed class EveDashboardServer(int port, EveDashboardProviderCatalog providers, EveDashboardCultNetPublisher cultNetPublisher, IdunnRudpHealthOptions idunnRudpHealth) : IDisposable
{
    ...
    {
        var state = providers.Get(activeProviderId).State;
        QueueIdunnRudpHealthIfDue(state, DateTimeOffset.UtcNow);
    ...
    private void QueueIdunnRudpHealthIfDue(DashboardState state, DateTimeOffset observedAt)
    {
...
{
    [Key(0)] public string DaemonId { get; set; } = "mimir-eve-dashboard";
    [Key(1)] public string State { get; set; } = "active";
    [Key(2)] public string Detail { get; set; } = "";
    [Key(3)] public string ObservedAt { get; set; } = "";
    [Key(4)] public string HealthContract { get; set; } = "mimir.cultnet-rudp-provider-health";
...
internal static class IdunnRudpHealthPublisher
{
    ...
    public static void Publish(string endpoint, IdunnDaemonHealthRecord health)
    {
        ...
        {
            throw new TimeoutException($"timed out connecting Mimir Eve dashboard RUDP health publisher to {endpoint}");
        }
        ...
        {
            MessageId = $"mimir-eve-dashboard-health:{health.DaemonId}:{observedAt.Replace(':', '-')}",
            Document = new CultNetRawDocumentRecord
            {
                ...
                SourceRuntimeId = "mimir-eve-dashboard",
