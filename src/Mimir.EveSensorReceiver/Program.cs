...
internal sealed record EveSensorReceiverOptions(
    int Port,
    string Path,
    string SourceId,
    string ExpectedType)
{
    ...
    {
        return new EveSensorReceiverOptions(
    ...
    {
        return int.TryParse(ParseString(args, name, ""), out var value) ? value : fallback;
    }
}

internal sealed class EveSensorReceiver(EveSensorReceiverOptions options) : IDisposable
{
    ...
    {
        listener.Start();
        Console.Error.WriteLine($"Eve sensor receiver listening on ws://0.0.0.0:{options.Port}{options.Path} source={options.SourceId}");
    ...
    private bool TryValidateFrameEvent(string text, out string normalized)
    {
        normalized = "";
        try
        {
            ...
            if (!root.TryGetProperty("type", out var type)
                || !string.Equals(type.GetString(), options.ExpectedType, StringComparison.OrdinalIgnoreCase))
            {
                return false;
            }
    ...
    {
        return stream.WriteAsync(new byte[] { 0x88, 0x00 }).AsTask();
    }
