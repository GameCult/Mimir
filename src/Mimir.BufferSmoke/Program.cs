...
if (args.Any(arg => string.Equals(arg, "--quest-usb-preflight-smoke", StringComparison.OrdinalIgnoreCase)))
{
    return await RunQuestUsbPreflightSmokeAsync(
        ParseStringOption(args, "--output", "artifacts/move-calibration/quest-usb-preflight.cc"))
        .ConfigureAwait(false);
}
...
static async Task<int> RunQuestUsbPreflightSmokeAsync(string outputPath)
{
    var devices = await RunProcessCaptureAsync("adb", ["devices", "-l"]).ConfigureAwait(false);
    if (devices.ExitCode != 0)
    {
        Console.Error.WriteLine($"quest-usb-preflight-smoke failed: adb devices exited {devices.ExitCode}: {devices.Stderr.Trim()}");
        return 1;
    }

    var quest = FindAuthorizedQuestDevice(devices.Stdout);
    if (quest is null)
    {
        Console.Error.WriteLine("quest-usb-preflight-smoke failed: no authorized Quest device found in adb devices -l");
        Console.Error.WriteLine(devices.Stdout.Trim());
        return 1;
    }

    var serial = quest.Value.Serial;
    var fields = quest.Value.Fields;
    ...
    var transportId = fields.GetValueOrDefault("transport_id", "");
    var androidRelease = await GetAdbPropertyAsync(serial, "ro.build.version.release").ConfigureAwait(false);
    var androidSdk = await GetAdbPropertyAsync(serial, "ro.build.version.sdk").ConfigureAwait(false);
    var buildFingerprint = await GetAdbPropertyAsync(serial, "ro.build.fingerprint").ConfigureAwait(false);
    var batterySummary = await GetAdbBatterySummaryAsync(serial).ConfigureAwait(false);
    var document = MimirQuestUsbPreflight.Create(
        serial,
        "device",
        product,
        model,
    ...
    await cache.UpsertAsync(
        document,
        new CultRecordHandle<MimirQuestUsbPreflightDocument>(new CultRecordKey($"mimir-quest-usb-preflight:{document.Serial}")))
        .ConfigureAwait(false);
    await cache.FlushAsync().ConfigureAwait(false);

    Console.WriteLine(
        $"quest-usb-preflight-smoke path={outputPath} serial={document.Serial} model={document.Model} product={document.Product} android={document.AndroidRelease} sdk={document.AndroidSdk} pose={document.PoseCaptureStatus}");
    return 0;
}

static (string Serial, Dictionary<string, string> Fields)? FindAuthorizedQuestDevice(string adbDevicesOutput)
{
    foreach (var rawLine in adbDevicesOutput.Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries))
    {
        var tokens = rawLine.Split(' ', StringSplitOptions.RemoveEmptyEntries);
        if (tokens.Length < 2 || tokens[0] == "List" || !string.Equals(tokens[1], "device", StringComparison.Ordinal))
        {
            continue;
        }
        ...
        var model = fields.GetValueOrDefault("model", "");
        var product = fields.GetValueOrDefault("product", "");
        if (model.Contains("Quest", StringComparison.OrdinalIgnoreCase) ||
            product.Contains("hollywood", StringComparison.OrdinalIgnoreCase))
        {
...
static async Task<string> GetAdbPropertyAsync(string serial, string property)
{
    var result = await RunProcessCaptureAsync("adb", ["-s", serial, "shell", "getprop", property], 5_000).ConfigureAwait(false);
    return result.ExitCode == 0 ? result.Stdout.Trim() : "";
}

static async Task<string> GetAdbBatterySummaryAsync(string serial)
{
    var result = await RunProcessCaptureAsync("adb", ["-s", serial, "shell", "dumpsys", "battery"], 5_000).ConfigureAwait(false);
    if (result.ExitCode != 0)
    {
        return "";
    }

    var usefulLines = result.Stdout.Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries)
        .Select(line => line.Trim())
        .Where(line =>
            line.StartsWith("AC powered:", StringComparison.OrdinalIgnoreCase) ||
            ...
            line.StartsWith("scale:", StringComparison.OrdinalIgnoreCase) ||
...
static async Task<(int ExitCode, string Stdout, string Stderr)> RunProcessCaptureAsync(
    string fileName,
    ...
    int timeoutMilliseconds = 10_000)
{
    ...
    var completed = await Task.WhenAny(waitTask, Task.Delay(timeoutMilliseconds)).ConfigureAwait(false);
    if (completed != waitTask)
    {
        ...
        return (-1, await stdoutTask.ConfigureAwait(false), $"timed out after {timeoutMilliseconds} ms");
    }
