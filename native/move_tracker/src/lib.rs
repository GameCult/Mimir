...
pub const MIMIR_MOVE_TRACKER_OK: c_int = 0;
pub const MIMIR_MOVE_TRACKER_NULL: c_int = -1;
pub const MIMIR_MOVE_TRACKER_INVALID: c_int = -2;
...
#[unsafe(no_mangle)]
pub extern "C" fn mimir_move_tracker_dispatch_plan(
    config: MoveTrackerConfig,
    out_plan: *mut MoveTrackerDispatchPlan,
) -> c_int {
    if out_plan.is_null() {
        return MIMIR_MOVE_TRACKER_NULL;
    }

    let Some(plan) = dispatch_plan(config) else {
        return MIMIR_MOVE_TRACKER_INVALID;
    };

    unsafe {
        *out_plan = plan;
    }
    MIMIR_MOVE_TRACKER_OK
}

#[unsafe(no_mangle)]
pub extern "C" fn mimir_move_tracker_extract_luma_cpu(
    config: MoveTrackerConfig,
    frame: *const u8,
    frame_len: usize,
