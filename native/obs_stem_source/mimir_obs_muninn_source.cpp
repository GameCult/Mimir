...
constexpr const char *MuninnObsCommandStorePath = "C:\\Meta\\Odin\\state\\muninn.obs.request.cc";
...
constexpr const char *MuninnOdinCultMeshUri = "cultmesh://odin/rendezvous/provider-catalog";
constexpr const char *MuninnOdinMediaTargetUri = "cultmesh://odin/media/muninn-raven-av";
constexpr const char *MuninnOdinRudpEndpoint = "127.0.0.1:17871";
...
static std::string quote_windows_arg(const std::string &value)
{
    if (value.empty()) {
        return "\"\"";
    }
...
static int run_muninn_cli(const std::string &exe, const std::vector<std::string> &args)
{
    _putenv_s("CULTMESH_URI_ODIN_RUDP", MuninnOdinRudpEndpoint);
    std::string command = quote_windows_arg(exe.empty() ? MuninnCommandExePath : exe);
...
static void request_stream_activation(MuninnSource *ctx, const std::string &stream_id, bool force = false)
{
    ...
    if (!ctx || canonical_stream_id.empty() || canonical_stream_id == "discovery-missing") {
        return;
    }
    const auto now = std::chrono::steady_clock::now();
    if (!force &&
    ...
    const auto command_exe = ctx->command_exe_path;
    const auto command_store = std::string(MuninnObsCommandStorePath);
    const auto store_path = ctx->store_path;
    const auto host_id = ctx->host_id;
    const auto media_receiver_host = ctx->media_target_host;
    const auto media_port = ctx->media_port;
    const auto media_packet_bytes = ctx->media_packet_bytes;
    const auto video_bitrate = ctx->rudp_video_bitrate_kbps;
    const auto latency_budget = ctx->rudp_latency_budget_ms;
    const auto video_source_id = ctx->video_source_id;
    const auto audio_source_id = ctx->audio_source_id;
    const bool wants_video = source_wants_video(ctx);
    const bool wants_audio = source_wants_audio(ctx);
    std::thread([command_exe,
                 command_store,
                 store_path,
                 host_id,
                 canonical_stream_id,
        ...
        std::vector<std::string> args = {
            ...
            "rudp",
            "--media-packet-bytes",
            std::to_string(media_packet_bytes),
            "--rudp-video-bitrate-kbps",
            std::to_string(video_bitrate),
            "--rudp-latency-budget-ms",
            std::to_string(latency_budget),
            "--odin-cultmesh-uri",
            MuninnOdinCultMeshUri,
        };
        if (!wants_video) {
            args.push_back("--no-video");
        } else if (!video_source_id.empty() && video_source_id != MuninnDisabledVideoSourceId) {
            args.push_back("--video-source");
            args.push_back(video_source_id + "=" + video_source_id);
        }
        if (!wants_audio) {
            args.push_back("--no-audio");
        } else if (!audio_source_id.empty() && audio_source_id != MuninnDisabledAudioSourceId) {
            args.push_back("--audio-source-id");
            args.push_back(audio_source_id);
        }
        if (run_muninn_cli(command_exe, args) != 0) {
            blog(LOG_WARNING, "Muninn Stream failed to publish Odin capture request");
        }
    }).detach();
    blog(LOG_INFO,
         "Muninn Stream requested activation for %s video_source=%s audio_source=%s through Odin %s video_bitrate_kbps=%u latency_budget_ms=%u audio_reorder_ms=%u",
...
static void refresh_catalog(MuninnSource *ctx)
{
    ...
    if (ctx->catalog_options.empty()) {
        refresh_obs_catalog_from_odin(ctx);
