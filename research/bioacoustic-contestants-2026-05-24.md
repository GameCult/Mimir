...
`artifacts/bioacoustic-contestants/contestants-20260524-192407/contestant-summary.json`
...
```text
packet-razor clean-roundtrip   score=59.282 payload_bitrate=14.6 bps payload=0.913 timing=1.000 frequency=0.935
packet-razor blur-light        score=59.295 payload_bitrate=14.6 bps payload=0.913 timing=1.000 frequency=0.953
packet-razor warp-light        score=13.658 payload_bitrate=11.7 bps payload=0.730 timing=0.667 frequency=0.683
packet-razor warp-light-blur   score=38.355 payload_bitrate=12.8 bps payload=0.800 timing=0.817 frequency=0.927
packet-razor warp-heavy-blur   score=28.960 payload_bitrate=10.7 bps payload=0.671 timing=1.000 frequency=0.911
```
...
That raised the honest score and made payload survive most warp/blur cases. The
current capped-delay receipt peaks at `59.295`, while an earlier uncapped run hit
`84.086` but let warp-light timing collapse. A 3-bit variant did not earn its
keep; clean was similar, but degraded payload recovery collapsed. The earlier
8-bit and 4-bit readings were schedule-entangled and are no longer treated as
bitrate evidence.

The leaderboard is now split:

- clean maximum: `canary-packet-trill + packet-razor-index`, `59.282`
- best overall: `canary-packet-trill + packet-razor-index` under blur-light,
  `59.295`
- remaining failure: razor's warp-light timing fit is improved but still weak
  at `0.667` timing accuracy with low frequency confidence.
