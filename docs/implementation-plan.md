...
`mimir.fensalir_reservoir_pressure.v1` is the pressure readout for the same
owner: Well poll/publish pressure, capture bytes, ready-slice ratio, timing
confidence, max distance from presentation, GPU queue milliseconds, and drop
causes for missing bodies, unavailable fences, or degraded timing.
...
- Mimir's active proof path no longer uses the direct
  ...
  in image pixels plus local clipspace/frustum coordinates, temporal-code/index
  stability flags, and shared calibration/spline identity.
  ...
  computes a thresholdable tiny-FFT onset novelty function using the old
  Perlines-style adaptive whitening pattern: fast/slow scrubbed spectrum,
  positive delta scrubbed against slow delta, rapidly decaying adaptive min/max
  ...
  and Raven Realtek loopback from one muxed Raven NVENC/AAC MPEG-TS SRT stream
  on port 5200. Starfire demuxes that single Raven transport locally into
  `raven-display` on port 5210 and `raven-realtk-loopback` f32 PCM on port
  5212 because Mimir's current source adapters ingest raw video and PCM
  separately. Scarlett ASIO channels remain labeled as shotgun/cardioid/local
  ...
  `MimirFfmpegPcmAudioStreamSource` is the network PCM audio owner after local
  Raven demux. `config/mimir-runtime.move-stream.local.json` remains a
